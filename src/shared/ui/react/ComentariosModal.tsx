...
const ComentarioCard: React.FC<ComentarioCardProps> = ({
  comment,
  currentUser,
  onAddChild,
  onDelete,
  ...
  const [replies, setReplies] = useState<ComentarioHijo[]>(
    comment.children || [],
  );
  ...
  return (
    <div
      className={`flex space-x-3 p-3 transition duration-150 border-b border-gray-100`}
    >
        ...
        <div className="mt-2 flex items-center space-x-4">
          {replies.length > 0 && (
            <button
              ...
              className="flex items-center text-xs text-blue-500 hover:text-blue-600 font-medium transition-colors cursor-pointer"
            >
          ...
          <form onSubmit={handleSubmitReply} className="mt-2">
            ...
            <div className="mt-2 flex justify-end space-x-2">
              ...
              <button
                ...
                className="cursor-pointer px-3 py-1 text-xs bg-blue-600 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="mt-3 ml-9 space-y-2">
            {replies.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="flex space-x-3 p-2 bg-gray-50 rounded-md"
                >
  ...
  const fetchComentariosPadres = useCallback(async () => {
    ...
    try {
      ...
      if (parentIds.length > 0) {
        ...
        const commentsWithChildren: ComentarioPadre[] = initialComments.map(
          (c) => ({
            ...c,
  ...
  const performDelete = async (id: string, isChild: boolean) => {
    try {
      ...
      if (isChild) {
        ...
        const { data: removedChildren, error: childErr } = await supabase
          .from("comentarios_hijos")
          .delete()
          .eq("id_comentario", id)
          .select();
          ...
          {!loading &&
            comentariosPadres.map((comment) => (
              <ComentarioCard
                ...
                onDelete={(id, isChild) => {
                  setDeleteTarget({ id, isChild: !!isChild });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="bg-white rounded-lg p-6 z-10 max-w-md w-full mx-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                ¿Seguro que deseas eliminar este comentario? Esta acción no se
                puede deshacer.
              </p>
              <div className="flex justify-end space-x-2">
                ...
                <button
                  onClick={() =>
                    performDelete(deleteTarget.id, deleteTarget.isChild)
                  }
                  disabled={isDeleting}
