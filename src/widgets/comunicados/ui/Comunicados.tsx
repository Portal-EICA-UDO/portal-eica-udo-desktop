  ...
  const handleCreateComunicado = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (newImageFile) {
        ...
        let filename = `${Date.now()}_${newImageFile.name}`.replace(
          /\s+/g,
          "_",
        );
      ...
      if (inserted) {
        ...
        if (currentPage === 1) {
          ...
          setCurrentPage(1);
          setData((prev) =>
            [inserted as ComunicadoItem, ...prev].slice(0, pageSize),
          );
        }
  ...
  const handleDeleteComunicado = async (
    ...
    try {
      ...
      const {
        ...
        error: deleteError,
        status,
      } = await supabase.from("comunicados").delete().eq("id", id).select();

      if (deleteError) {
        // Mostrar mensaje de error más explícito
        const msg =
          (deleteError && (deleteError.message || deleteError.details)) ||
          "Error desconocido al eliminar el registro.";
      ...
      alert(
        "Error al eliminar comunicado. Revisa la consola para más detalles.",
      );
    } finally {
      setLoading(false);
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center justify-between">
              <div
                ...
                onClick={() => {
                  setIsModalOpen(true);
                  setComunicadoID(item.id.toString());
                }}
              >
                <svg
                  className="w-4 h-4"
                ...
                >
                  <path
                    ...
                    strokeWidth="2"
                    d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.805A9.73 9.73 0 0112 3c4.97 0 9 3.582 9 8z"
                  />
                </svg>
              </div>
              {($role === "admin" || $role === "superAdmin") && currentUser && (
                <button
                  type="button"
                  onClick={() => {
                    setDeleteTarget({ id: item.id, imagen: item.imagen_url });
                    setDeleteModalOpen(true);
                  }}
                  className="text-red-500 hover:text-red-700 ml-auto flex items-center space-x-2 cursor-pointer"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
      ...
      <div className="flex justify-center items-center space-x-2 p-4 border-t border-gray-100 mt-4">
        ...
        {pageNumbers.map((number) => (
          <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded-md cursor-pointer transition-colors duration-150 ${
              number === currentPage
    ...
    return (
      <div className="col-span-full text-center py-12">
        ...
        <p className="text-gray-600">
          {error.message || "Error desconocido. Intente recargar la página."}
        </p>
      </div>
    );
  }
          ...
          <div className="p-4 border-b">
            <div className="flex items-start justify-between">
              ...
              <button
                ...
                className="px-3 py-1 text-sm bg-blue-600 text-white cursor-pointer rounded-md hover:bg-blue-700 transition"
              >
                {isCreateOpen ? "Cerrar" : "Nuevo"}
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-lg p-6 z-10 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              ¿Seguro que deseas eliminar este comunicado? Esta acción no se
              puede deshacer.
            </p>
            <div className="flex justify-end space-x-2">
              ...
              <button
                onClick={() =>
                  handleDeleteComunicado(deleteTarget.id, deleteTarget.imagen)
                }
