...
export const CreateDependences: React.FC<Props> = ({
  onSuccess,
  schools,
  degrees,
  staff,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      reset();
      setSuccessMsg("Dependencia creada correctamente");
    } catch (error) {
      console.error(error);
      setErrorMsg("Error al crear la dependencia");
    }
  };
  return (
    <section className="  p-6 bg-white rounded-lg w-[clamp(300px,calc(100vw-77px),672px)]">
      ...
      <form onSubmit={handleRegisterSubmit(onSubmit)}>
        ...
        <div className="flex justify-end mt-6">
          ...
          >
            {isSubmitting ? "Cargando..." : "Crear Dependencia"}
          </button>
        </div>
      </form>
    </section>
