...
const StaffGrid = () => {
  ...
  const [staffWithMaterias, setStaffWithMaterias] = useState<
    StaffWithMaterias[]
  >([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (staffList && staffList.length > 0) {
          const { data: allPensumRelations, error: pensumError } =
            await supabase.from("pensum").select("*");
          ...
          if (materiaIds.length > 0) {
            const { data: materiasFromDB, error: materiasError } =
              await supabase
                .from("materias")
                .select(
                  `
          ...
          materiasConCarreras.forEach((materia) => {
            const carreraData = Array.isArray(materia.carreras)
              ? materia.carreras[0]
              : materia.carreras;
    ...
    return (
      <div className="col-span-full text-center py-12">
        ...
        <p className="text-gray-600">
          La base de datos no contiene registros de staff.
        </p>
      </div>
    );
  }
